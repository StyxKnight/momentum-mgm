    ...
    slides_data = [
        {
            ...
            "body": [
                ...
                f"ADI Score: {adi.get('score', 'N/A')} — worse than {int((adi.get('score', 0) or 0) * 100)}% of Montgomery neighborhoods",
                f"SVI Score: {svi.get('score', 'N/A')}",
                f"EJI Score: {eji.get('score', 'N/A')}",
                "",
        ...
        {
            "title": "Deprivation Scores",
            "body": [
                f"Area Deprivation Index (ADI): {adi.get('score', 'N/A')} — {adi.get('severity', '')}",
                f"  Worse than {adi.get('percentile_rank', 'N/A')}% of Montgomery tracts",
                "",
                f"Social Vulnerability Index (SVI): {svi.get('score', 'N/A')} — {svi.get('severity', '')}",
                f"  Worse than {svi.get('percentile_rank', 'N/A')}% of Montgomery tracts",
                "",
                f"Environmental Justice Index (EJI): {eji.get('score', 'N/A')} — {eji.get('severity', '')}",
                f"  Worse than {eji.get('percentile_rank', 'N/A')}% of Montgomery tracts",
            ],
        },
        {
            "title": "Federal Programs Available",
            "body": [
                f"• {p.get('name', '')} ({p.get('agency', '')})\n  {p.get('amount') or 'Amount varies'} — {p.get('eligibility', '')[:80]}"
                for p in programs
        ...
        },
        {
            ...
            "body": [
                f"• {c.get('city', '')}: {c.get('approach', '')[:100]}\n  Result: {c.get('outcome', '')[:80]}"
                for c in cities
            ] if cities else ["No comparable city data available."],
        },
        {
            "title": "Recommended Actions",
            "body": [f"• {r[:120]}" for r in recs] if recs else ["See find_solutions() for recommendations."],
